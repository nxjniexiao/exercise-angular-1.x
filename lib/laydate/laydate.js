  ...
  */
  ...
  Class.prototype.position = function() {
    var that = this,
      ...
      elemWidth = options.type === 'time' ? 162 : that.elem.offsetWidth, //控件的宽度
      elemHeight = options.type === 'time' ? 205 : that.elem.offsetHeight, //控件的高度
      //滚动条高度
    ...
    if (type === 'year') {
        ...
        lay(ul)
          ...
          .each(function(i, ol) {
            lay(ol)
              .find('li')
              .each(function(ii, li) {
                if (i === 1 && options.minutesGradient) {
                  ii = ii * options.minutesGradient; // 乘以分钟梯度
                }
                that.limit(
                  lay(li),
                  [
      ...
      lay.each([24, 60, 60], function(i, item) {
        ...
        lay.each(new Array(item), function(ii) {
          // 分钟梯度，若为5，则只显示0/5/15...
          if (
            options.minutesGradient &&
            i === 1 &&
            ii % options.minutesGradient !== 0
          ) {
            return;
          }
          childUL.push(
          ...
          lay(ul)
            ...
            .each(function(i) {
              var ol = this,
                li = lay(ol).find('li');
              if (i === 1 && options.minutesGradient) {
                ol.scrollTop = 30 * (that[startEnd][hms[i]] / options.minutesGradient - 2);
              } else {
